...
if s-l <= 1.65:#проверяем, чтобы s и l отличались не более чем на 1.65
    ...
    while True:
        ...
        if t <= sum:
            ...
            k -= 1#нам надо t меньше суммы, поэтому убираем одну итерацию
    ...
    output = open('output.txt', 'w')
    output.write(str('ERROR! Разность второго и первого вводимых числе не должна отличаться на более чем 1.65'))
